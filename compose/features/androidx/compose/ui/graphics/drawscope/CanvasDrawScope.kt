...
class CanvasDrawScope : DrawScope {
    ...
    internal data class DrawParams(
        ...
        var canvas: Canvas = object : Canvas {
            override fun save() {}
            ...
            override fun drawImage(image: ImageBitmap, topLeft: Offset, paint: Paint) {}
            ...
            override fun drawVertices(vertices: androidx.compose.ui.graphics.Vertices, blendMode: BlendMode, paint: Paint) {}
    ...
    object : DrawTransform {
        ...
        override fun inset(left: Float, top: Float, right: Float, bottom: Float) {
            this@asDrawTransform.canvas.let {
                val l = if (left.isFinite()) left else 0f
                val t = if (top.isFinite()) top else 0f
                val r = if (right.isFinite()) right else 0f
                val b = if (bottom.isFinite()) bottom else 0f
                val sw = if (size.width.isFinite()) size.width else 0f
                val sh = if (size.height.isFinite()) size.height else 0f
                val updatedWidth = (sw - (l + r)).coerceAtLeast(0f)
                val updatedHeight = (sh - (t + b)).coerceAtLeast(0f)
                this@asDrawTransform.size = Size(updatedWidth, updatedHeight)
                it.translate(l, t)
            }
        }
